...
export default function Products() {
  ...
  const openEdit = async (p: Product) => {
    ...
    (imgs || []).forEach((img) => {
      if (img.sort_order < 5) previews[img.sort_order] = img.image_url;
    });
  ...
  return (
    <div className="space-y-6 animate-fade-in">
                ...
                products.map((product) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <span
                          className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${product.is_active ? "bg-success/10 text-success" : "bg-muted text-muted-foreground"}`}
                        >
      ...
      {showModal && (
        <div
          className="fixed inset-0 z-50 flex justify-center bg-background/80 backdrop-blur-sm p-4 !mt-0"
          onClick={closeModal}
        >
          <div
            className="w-full max-w-[95vw] sm:max-w-md md:max-w-lg lg:max-w-2xl rounded-lg border border-border bg-card shadow-xl animate-fade-in max-h-[95vh] md:max-h-fit lg:max-h-fit flex flex-col"
            ...
            <div className="space-y-3 sm:space-y-4 p-4 sm:p-6 overflow-y-auto">
              ...
              <div>
                    ...
                    >
                      {imagePreviews[idx] ? (
                        <>
                          ...
                          {idx === 0 && (
                            <span className="absolute bottom-0 left-0 right-0 bg-primary/80 text-center text-[10px] text-white py-0.5">
                              Primary
                            </span>
                          )}
                        </>
                ...
                <p className="mt-1 text-[10px] sm:text-xs text-muted-foreground">
                  First image will be set as primary. JPG, PNG, WebP supported.
                </p>
              </div>
              ...
              <button
                ...
                disabled={
                  !form.name || !form.base_price || saveMutation.isPending
                }
